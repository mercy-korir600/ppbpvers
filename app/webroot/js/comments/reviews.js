$(function() {
    ...
    var trWrapper = '\
          <div class="row-fluid attacho">\
            <div class="span10"><input name="data[Attachment][{i}][id]" id="attachments-{i}-id" type="hidden"> \
                ...
                <input type="hidden" id="attachments-{i}-model" value="Review" name="data[Attachment][{i}][model]" style="display: inline;">\
    ...
    $(".addUpload").click(function() {
      ...
      if ($(this).closest('div.reviewUploads').children('div.attacho').length < 7) {            
          trVar = $.parseHTML(trWrapper.replace(/{i}/g, intId).replace(/{n}/g, name));
          $(this).closest("div.reviewUploads").append(trVar);
      } else {
