...
class RemindersShell extends AppShell
{
  ...
  public $uses = array('Aefi', 'Sadr', 'Pqmp', 'Device', 'Medication', 'Transfusion', 'Message', 'Reminder', 'Aggregate');

  public function main()
  {


    // Get all Aggregate Reports due today

    $reports = $this->Aggregate->find(
      'all',
      array(
        'contain' => array('User'),
        'conditions' => array(
          'Aggregate.submitted' => '2',
          'Aggregate.reminder_date IS NOT NULL',
          'DATE(Aggregate.reminder_date)' => date("Y-m-d")
        )
      )
    );
    $this->out('This is a list of reports');
    foreach ($reports as $rp) {
      $this->out($rp['Aggregate']['reference_no']);

      $id = $rp['Aggregate']['id'];

      $html = new HtmlHelper(new ThemeView());
      $message = $this->Message->find('first', array('conditions' => array('name' => 'mah_reminder_email')));
      $variables = array(
        'name' => $rp['User']['name'],
        'reference_no' => $rp['Aggregate']['reference_no'],
        'type' => 'PSUR',
        'created' => $rp['Aggregate']['created'],
        'reference_link' => $html->link(
          $rp['Aggregate']['reference_no'],
          array(
            'controller' => 'aggregates',
            'action' => 'view', $rp['Aggregate']['id'],
            'reporter' => true,
            'full_base' => true
          ),
          array('escape' => false)
        ),
        'modified' => $rp['Aggregate']['modified']
      );
      $datum = array(
        'email' => $rp['Aggregate']['reporter_email'],
        'user_id' => $rp['Aggregate']['user_id'],
        'type' => 'mah_reminder_email',
        'model' => 'Aggregate',
      ...
      $rem = array();
      $rem['foreign_key'] = $rp['Aggregate']['id'];
      $rem['user_id'] = $rp['Aggregate']['user_id'];
      $rem['model'] = 'Aggregate';
      $rem['reminder_type'] = 'mah_reminder_email';
      $this->Reminder->save($rem);


      $today = new DateTime();

      if ($rp['Aggregate']['submission_frequency'] === "0") {
        $interval = new DateInterval('P' . $rp['Aggregate']['interval_code'] . 'M');
        $today->add($interval);
      } else {
        $interval = new DateInterval('P' . $rp['Aggregate']['interval_code'] . 'Y');
        $today->add($interval);
      }
      $reminder_date = $today->format("Y-m-d H:i:s");

      $aggregate = $this->Aggregate->read(null, $id);
      $this->Aggregate->saveField('reminder_date', $reminder_date);
    }
    exit;
      ...
      if (!empty($aefi['Aefi']['reporter_email'])) {
        ...
        $variables = array(
          ...
          'reference_link' => $html->link(
            ...
            array('controller' => 'aefis', 'action' => 'edit', $aefi['Aefi']['id'], 'reporter' => true, 'full_base' => true),
            array('escape' => false)
          ),
      ...
      if (!empty($sadr['Sadr']['reporter_email'])) {
        ...
        $variables = array(
          ...
          'reference_link' => $html->link(
            ...
            array('controller' => 'sadrs', 'action' => 'edit', $sadr['Sadr']['id'], 'reporter' => true, 'full_base' => true),
            array('escape' => false)
          ),
      ...
      if (!empty($pqmp['Pqmp']['reporter_email'])) {
        ...
        $variables = array(
          ...
          'reference_link' => $html->link(
            ...
            array('controller' => 'pqmps', 'action' => 'edit', $pqmp['Pqmp']['id'], 'reporter' => true, 'full_base' => true),
            array('escape' => false)
          ),
      ...
      if (!empty($device['Device']['reporter_email'])) {
        ...
        $variables = array(
          ...
          'reference_link' => $html->link(
            ...
            array('controller' => 'devices', 'action' => 'edit', $device['Device']['id'], 'reporter' => true, 'full_base' => true),
            array('escape' => false)
          ),
    ...
    //MEDICATIONS
    $medications = $this->Medication->find(
      'all',
      array(
      ...
      if (!empty($medication['Medication']['reporter_email'])) {
        ...
        $variables = array(
          ...
          'reference_link' => $html->link(
            ...
            array('controller' => 'medications', 'action' => 'edit', $medication['Medication']['id'], 'reporter' => true, 'full_base' => true),
            array('escape' => false)
          ),
    ...
    //TRANSFUSIONS
    $transfusions = $this->Transfusion->find(
      'all',
      array(
      ...
      if (!empty($transfusion['Transfusion']['reporter_email'])) {
        ...
        $variables = array(
          ...
          'reference_link' => $html->link(
            ...
            array('controller' => 'transfusions', 'action' => 'edit', $transfusion['Transfusion']['id'], 'reporter' => true, 'full_base' => true),
            array('escape' => false)
          ),
